...
#include <pameco/COptions.h>
#include <pameco/SMetrics.h>
...
PMCJ(struct table_t * relR, struct table_t * relS, joinconfig_t * config)
{
    ...
    {
        logger(INFO, "Memory constraint = %d MB", options.getMemoryConstraintMB());
        PMC::CMemoryManagement memManagement;
//        double bias = options.getBias();
//        bool isBiasZero =
//                (std::fabs(bias - 0.0) <= std::numeric_limits<double>::epsilon() * std::fmax(std::fabs(bias), std::fabs(0.0)));
    ...
    double throughput = (double) (metrics.r_cardinality + metrics.s_cardinality)  / (double) metrics.time_total_us;
...
{
    uint32_t output_size = (uint32_t)(sizeof(relation_t) + rel->num_tuples * (sizeof(row_t)));
    ...
    if (seal_chunk_size != 0) {
        ...
        uint32_t * tmp;
        uint32_t available_space = (uint32_t)(seal_chunk_size * 1024 - sizeof(sgx_sealed_data_t) -
...
#ifdef PCM_COUNT
    struct rusage_reduced_t usage{0,0,0,0,0,0,0,0};
                                  ...
                                  uint32_t seal_chunk_size)
{
    ...
    result_t result;
    ecall_join(&result, relR, relS, algorithm, config);
    ...
    logger(INFO, "uns_timer = %lu (%.2lf%%)",
           unseal_timer, (double) unseal_timer*100.0/(double)(unseal_timer + seal_timer));
    logger(INFO, "s_timer   = %lu (%.2lf%%)",
           seal_timer, (double) seal_timer*100.0/(double)(unseal_timer + seal_timer));
...
//    seal_relation(relS);
    ...
    logger(INFO, "total sealing share = %.2lf%%",
           (double) (unseal_timer+seal_timer)*100.0/(double)(unseal_timer+seal_timer+join1_timer+join2_timer));
